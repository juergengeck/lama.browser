...
// TODO: These need to be imported from a quicvc-protocol module
// Currently using placeholder types from types.ts
// Original import: '@refinio/one.core/lib/quicvc-protocol/index.js'
/*
import {
    QuicPacketType,
    QuicFrameType,
...
// Placeholder - these functions need to be implemented or injected
const buildLongHeaderPacket: any = null;
const buildShortHeaderPacket: any = null;
const parseQuicPacketHeader: any = null;
const parseFrame: any = null;
const decodeVarint: any = null;
const encodeVarint: any = null;
const QuicPacketType: any = {};
const QuicFrameType: any = {};
const QuicVCFrameType: any = {};
...
export class QuicVCConnectionManager {
    ...
    getConnection(deviceId: string): QuicVCConnection | undefined {
        for (const connection of this.connections.values()) {
            if (connection.deviceId === deviceId) {
                console.log(`[QuicVCConnectionManager] getConnection(${deviceId}) found connection:`, {
                    deviceId: connection.deviceId,
                    dcidLength: connection.dcid.length,
                    dcidHex: Array.from(connection.dcid).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '),
                    dcidAscii: String.fromCharCode(...Array.from(connection.dcid)),
                    scidLength: connection.scid.length,
        ...
        console.warn(`[QuicVCConnectionManager] getConnection(${deviceId}) - NO CONNECTION FOUND`);
        return undefined;
    }
    ...
    /**
     * Initialize with transport, VCManager, and QuicModel
     */
    async initialize(transport: IQuicTransport, vcManager: VCManager, quicModel: QuicModel, ownVC?: DeviceIdentityCredential): Promise<void> {
        this.vcManager = vcManager;
        this.quicModel = quicModel;

        // QuicModel should already be initialized by the app
        ...
        console.log('[QuicVCConnectionManager] Using provided QuicModel instance');
    ...
    async connect(deviceId: string, address: string, port: number, credential: any): Promise<void> {
        ...
        if (existingConnection) {
            ...
            // Don't close connections unnecessarily - causes wasteful reconnections
            // Only close if the connection is in a bad/stale state
            if (existingConnection.state !== 'initial') {
                console.log(`[QuicVCConnectionManager] Closing connection in ${existingConnection.state} state`);
                this.closeConnection(existingConnection, `Restarting from ${existingConnection.state} state`);
            } else {
                console.log(`[QuicVCConnectionManager] Connection already in initial state - will reuse`);
                // Don't create a new connection - fall through to reuse existing one
                existingConnection.localVC = credential;
                existingConnection.deviceId = deviceId;

                // Send VC_INIT on the existing connection
        ...
        console.log(`[QuicVCConnectionManager] Created connection with credential subject ID: ${credential?.credentialSubject?.id || 'MISSING'}`);
        ...
        console.log(`[QuicVCConnectionManager] ===== Created CLIENT connection ${connId} =====`);
        console.log(`[QuicVCConnectionManager] Device: ${deviceId} at ${address}:${port}`);
        console.log(`[QuicVCConnectionManager] DCID length=${dcid.length}, SCID length=${scid.length}`);
        console.log(`[QuicVCConnectionManager] DCID (hex)=${Array.from(dcid).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' ')}`);
        console.log(`[QuicVCConnectionManager] SCID (hex)=${Array.from(scid).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' ')}`);
        console.log(`[QuicVCConnectionManager] ========================================`);
    ...
    private async sendInitialPacket(connection: QuicVCConnection): Promise<void> {
        ...
        console.log('[QuicVCConnectionManager] Sending VC_INIT frame:', {
            ...
            credentialSubjectId: connection.localVC?.credentialSubject?.id || 'unknown',
    ...
    private async handleNewConnection(header: QuicVCPacketHeader, rinfo: { address: string, port: number }): Promise<QuicVCConnection> {
        console.log(`[QuicVCConnectionManager] handleNewConnection - Creating SERVER connection from incoming packet`);
        console.log(`[QuicVCConnectionManager] Incoming header DCID length=${header.dcid.length}, SCID length=${header.scid.length}`);
        console.log(`[QuicVCConnectionManager] Incoming DCID (will be our SCID):`, Array.from(header.dcid).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '));
        console.log(`[QuicVCConnectionManager] Incoming SCID (will be our DCID):`, Array.from(header.scid).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '));
        console.log(`[QuicVCConnectionManager] Incoming SCID as ASCII:`, String.fromCharCode(...Array.from(header.scid)));
        ...
        console.log(`[QuicVCConnectionManager] Created SERVER connection with DCID length=${connection.dcid.length}, SCID length=${connection.scid.length}`);
    ...
    private createProtectedPacket(connection: QuicVCConnection, frameData: Uint8Array): Uint8Array {
        ...
        console.log(`[QuicVCConnectionManager] Built SHORT header packet: ${packet.length} bytes`);
        console.log(`[QuicVCConnectionManager] Packet structure: flags(1) + dcid(${header.dcid.length}) + pn(2) + payload(${frameData.length})`);
        console.log(`[QuicVCConnectionManager] Expected total: ${1 + header.dcid.length + 2 + frameData.length} bytes`);
        ...
        if (connection.sessionKey && QuicVCConnectionManager.ENABLE_ENCRYPTION) {
            ...
            console.log(`[QuicVCConnectionManager] DCID length: ${connection.dcid.length}, CONNECTION_ID_LENGTH: ${this.CONNECTION_ID_LENGTH}`);
            console.log(`[QuicVCConnectionManager] Session key (first 16 bytes):`,
                Array.from(connection.sessionKey.slice(0, 16)).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '));
    ...
    private getQuicModel(): QuicModel {
        if (!this.quicModel) {
            throw new Error('[QuicVCConnectionManager] QuicModel not initialized. Call initialize() first.');
        }
