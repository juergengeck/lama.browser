...
import { IQuicTransport, DeviceIdentityCredential, VCPresentationMessage, NetworkServiceType, QuicTransportOptions, Device, DiscoveryMessage, UdpRemoteInfo, DiscoveryConfig } from './types.js';
...
export class DiscoveryProtocol {
  ...
  constructor(
    ...
    if (transport) {
      ...
      // Transport must be provided - we don't create our own in this port
      throw new Error('DiscoveryProtocol requires a transport to be provided');
    }
  ...
  public async init(): Promise<boolean> {
    ...
    try {
      ...
      // The external provider is responsible for its initialization
  ...
  private async sendDiscoveryRequest(): Promise<void> {
    ...
    try {
      ...
      if (typeof safeDeviceId !== 'string') {
        ...
        // Try to convert it
        safeDeviceId = String(safeDeviceId) || `unknown-${Math.random().toString(36).substring(2, 8)}`;
  ...
  private async handleDiscoveryRequest(request: DiscoveryMessage, rinfo: UdpRemoteInfo): Promise<void> {
    ...
    const device: any = {
      ...
      online: true,
      // LED status is set only on initial discovery, never updated from broadcasts
      // LED control is manual only
      blueLedStatus: (request.deviceType === 'ESP32' ? 'off' : undefined),
    ...
    if (existingDevice) {
      // Update existing device, preserving ownership, credential status, AND LED status
      this.devices.set(device.id, {
        ...
        // ALWAYS preserve LED status - LED control is manual only, never updated from broadcasts
        blueLedStatus: existingDevice.blueLedStatus
      });
  ...
  private handleDiscoveryResponse(response: DiscoveryMessage, rinfo: UdpRemoteInfo): void {
    ...
    const device: any = {
      ...
      online: true,
      // LED status is set only on initial discovery, never updated from responses
      // LED control is manual only
      blueLedStatus: (response.deviceType === 'ESP32' ? 'off' : undefined),
    ...
    if (existingDevice) {
      ...
      this.devices.set(device.id, {
        ...
        ownerId: existingDevice.ownerId,
        hasValidCredential: existingDevice.hasValidCredential,
        // ALWAYS preserve LED status - LED control is manual only, never updated from responses
        blueLedStatus: existingDevice.blueLedStatus
      });
