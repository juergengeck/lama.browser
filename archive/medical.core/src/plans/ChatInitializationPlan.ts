...
export class ChatInitializationPlan {
   ...
   */
  async createPatientPhysicianChat(
    patientId: SHA256IdHash<any>,
    ...
    try {
      ...
      try {
        ...
        console.warn('[ChatInitPlan] Could not send welcome message:', (msgError as Error).message);
   ...
   */
  async createPatientChatsWithPhysicians(
    patientId: SHA256IdHash<any>,
    physicianIds: SHA256IdHash<any>[],
    options: WelcomeMessageOptions = {}
  ): Promise<ChatInitResult[]> {
    console.log('[ChatInitPlan] Creating chats for patient with', physicianIds.length, 'physicians');

    const results: ChatInitResult[] = [];
   ...
   */
  private buildWelcomeMessage(options: WelcomeMessageOptions): string {
    if (options.customMessage) {
      return options.customMessage;
    }

    const parts: string[] = ['Welcome to Flexibel!'];
   ...
   */
  async assignAndCreateChat(
    patientId: SHA256IdHash<any>,
    defaultPhysicianId?: SHA256IdHash<any>
  ): Promise<ChatInitResult & { physicianId?: SHA256IdHash<any> }> {
